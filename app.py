...
from api import whatsappApi, SMSApi,EmailAPI
...
def upload_file():
    ...
    f = text("select farmer_info.Phone from farmer_info where farmer_info.email = :email")

    phone = db.engine.execute(f, email = email).fetchall()
    
    name = text("select Sign_up.FullName from Sign_up where Sign_up.email = :email")

    name = db.engine.execute(name, email = email).fetchall()

    

    msg = "Hello" + " " + name[0][0] + "\n\n" + "Your crop is predicted to be:" + " " + res + "\n\n" + "with a probability of:-" + " " + str(pred_prob) + "\n\n" + "Here is some information that might help you." + " " + wikiInfo




    try:
        whatsappApi(phone[0][0],msg)
    except:
        print("Error")    
...
def invoice_confirm(id,date,number):
    ...
    #rendered = render_template('report.html' ,Date = date,Invoice = number,Total = Total, Crop = Crop, sEmail = sEmail, bEmail = bEmail)

    print(bEmail,number,date,bEmail,Crop,Total,Total)

    try:
        EmailAPI(bEmail,number,date,bEmail,Crop,Total,Total)
    except:
        print("Error")
    ...
    if user[0].UType == 'wholesaler':
        ...
        u = text("select Phone from Wholesaler_info where Wholesaler_info.Email = :id")
    
        phone = db.engine.execute(u, id = bEmail).fetchall()

        name = text("select FullName from Sign_up where Sign_up.Email = :id")
    
        name = db.engine.execute(name, id = bEmail).fetchall()

        try:
            txt = "Dear "+str(name[0][0])+"\nThank you for using Cropwayy. A detailed invoice of your purchase has been sent to you over Email.\nFor more information visit: www.cropwayy.co"

            SMSApi(phone[0][0],txt)
        ...
        u = text("select Phone from Retailer_info where Retailer_info.Email = :id")
